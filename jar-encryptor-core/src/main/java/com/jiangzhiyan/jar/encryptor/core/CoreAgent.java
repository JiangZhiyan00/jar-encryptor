...
public class CoreAgent {
    ...
    public static void premain(String args, Instrumentation inst) {
        ...
        pwd = JarDecryptor.readPassFromJar(new File(Objects.requireNonNull(JarUtils.getRootPath(null))));
    ...
    public static char[] readPasswordFromFile(CmdLineOption options) {
        ...
        String configName = jarName.substring(0, jarName.length() - 3) + "jar-encryptor.txt";
        File config = new File(path, configName);
        if (!config.exists()) {
            config = new File(path, "jar-encryptor.txt");
        }
